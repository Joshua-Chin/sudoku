...
def _constraints():
    ...
    squares = variables.reshape((3, 3, 3, 3, 9))
    squares = np.transpose(squares, (0, 2, 4, 1, 3))
    constraint = squares.reshape((81, 9))
    constraints.append(constraint)
...
class Board(object):
    ...
    def __init__(self, givens=None):
        ...
        self.satisfied = np.zeros((324,), dtype=np.bool)
    ...
    def select(self, variable):
        ...
        self.satisfied[v2c[variable]] = True
    ...
    def deselect(self, variable):
        ...
        if self.eliminated[variable] != -1:
            raise ValueError('Attempted to deselect an unselected variable.')
        self.eliminated[variable] = 0
        self.satisfied[v2c[variable]] = False
        for neighbor in v2n[variable]:
            self.eliminated[neighbor] -= 1
            if not self.eliminated[neighbor]:
                self.options[v2c[neighbor]] += 1
    ...
    def pivot(self):
        """Selects a unsatisfied constraint with a minimal number of options."""
        pivot = None
        min_options = 10
